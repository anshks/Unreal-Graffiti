...
arduinoData=serial.Serial('/dev/ttyACM0', 4800)
...
yBlob = cv.imread('blackblob.png',-1)
...
paintWindow = cv.rectangle(paintWindow, (505,1), (600,65), (0, 0, 0), -1)
cv.putText(paintWindow, "CLEAR ALL", (49, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv.LINE_AA)
cv.putText(paintWindow, "SIZE 1", (185, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv.LINE_AA)
cv.putText(paintWindow, "SIZE 2", (298, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv.LINE_AA)
cv.putText(paintWindow, "SIZE 3", (420, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv.LINE_AA)
cv.putText(paintWindow, "SIZE 4", (520, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (150,150,150), 2, cv.LINE_AA)

cv.namedWindow("Paint", cv.WINDOW_NORMAL)
#cv.setWindowProperty("Paint",cv.WND_PROP_FULLSCREEN,cv.WINDOW_FULLSCREEN)

#pygame.mixer.music.load("Spray Sound Effect_Trim.mp3")
...
while True:
	...
	abc = myData.decode('utf-8',errors='replace')
	try:
		color_input=int(abc[1])
	except:
		continue	
	try:
		if abc[6]=='0':
	...
	frame = cv.rectangle(frame, (505,1), (600,65), (0, 0, 0), -1)
	cv.putText(frame, "CLEAR ALL", (49, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv.LINE_AA)
	cv.putText(frame, "SIZE 1", (185, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv.LINE_AA)
	cv.putText(frame, "SIZE 2", (298, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv.LINE_AA)
	cv.putText(frame, "SIZE 3", (420, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv.LINE_AA)
	cv.putText(frame, "SIZE 4", (520, 33), cv.FONT_HERSHEY_SIMPLEX, 0.5, (150,150,150), 2, cv.LINE_AA)
	...
	if len(cnts) > 0:
		...
		if center[1] <= 65:
			...
			if color == 0:
				if en == 1:
					paintWindow = overlay_transparent(paintWindow, bBlob, (55*size,55*size), cx-2.5, cy+2.5) # Blue
					tempWindow = paintWindow
				elif en == 0:
					tempWindow=paintWindow
				...
				if en == 1:
					paintWindow = overlay_transparent(paintWindow, gBlob, (55*size,55*size), cx-2.5, cy+2.5) # Green
					tempWindow = paintWindow
				elif en == 0:
					tempWindow=paintWindow
					paintWindow = overlay_transparent(paintWindow, crosshair, (55,55), cx-2.5, cy+2.5)
				...
				if en == 1:
					paintWindow = overlay_transparent(paintWindow, rBlob, (20*size,20*size), cx-2.5, cy+2.5) # Red
					tempWindow = paintWindow
				elif en == 0:
					tempWindow = paintWindow
					paintWindow = overlay_transparent(paintWindow, crosshair, (55,55), cx-2.5, cy+2.5)
				...
				if en == 1:
					paintWindow = overlay_transparent(paintWindow, yBlob, (55*size,55*size), cx-2.5, cy+2.5) # Yellow
					tempWindow = paintWindow
				elif en == 0:
					tempWindow = paintWindow
					paintWindow = overlay_transparent(paintWindow, crosshair, (55,55), cx-2.5, cy+2.5)	
